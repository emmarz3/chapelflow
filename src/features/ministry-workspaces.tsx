...
export function VolunteerWorkspacePage() {
  ...
  const profiles = useQuery({ queryKey: ["volunteer-profiles"], queryFn: volunteerService.profiles });
  ...
  const active = profiles.data.data.filter((profile) => profile.isActive && profile.status === "ACTIVE").length;
  const awaiting = assignments.data.data.filter((assignment) => assignment.status === "PENDING").length;
  return <>
    <PageHeader eyebrow="Volunteer operations" title="Duty roster" description="Assignments, conflicts, availability and service history remain validated by the server." actions={canUpdate ? <Button icon={<Plus />} onClick={() => setAssignmentOpen(true)}>Assign duty</Button> : undefined} />
    ...
    <Modal open={assignmentOpen} onClose={() => setAssignmentOpen(false)} title="Assign volunteer duty" description="The assignment begins pending; confirmation and service hours are recorded separately." footer={<><Button variant="ghost" onClick={() => setAssignmentOpen(false)}>Cancel</Button><Button type="submit" form="assignment-form" loading={createAssignment.isPending}>Create assignment</Button></>}><form id="assignment-form" className="form-grid" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); createAssignment.mutate({ volunteer: String(form.get("volunteer")), role: String(form.get("role")), notes: String(form.get("notes") || "") }); }}><label className="field field--full"><span>Volunteer</span><select name="volunteer" required defaultValue=""><option value="" disabled>Select an active volunteer</option>{profiles.data.data.filter((profile) => profile.isActive && profile.status === "ACTIVE").map((profile) => <option key={profile.id} value={profile.id}>{profile.memberName}</option>)}</select></label><label className="field"><span>Duty</span><select name="role" defaultValue="OTHER"><option value="USHER">Usher</option><option value="CHOIR">Choir</option><option value="MEDIA">Media</option><option value="SECURITY">Security</option><option value="TECHNICAL">Technical</option><option value="PROTOCOL">Protocol</option><option value="OTHER">Other</option></select></label><Field name="notes" label="Duty note" maxLength={255} />{createAssignment.isError && <p className="form-error field--full">{message(createAssignment.error)}</p>}</form></Modal>
  </>;
}
