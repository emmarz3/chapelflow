...
class ContentEntrySerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if content_type == ContentType.MEDIA and parent and parent.content_type != ContentType.GALLERY:
            raise serializers.ValidationError({"parent": ["Media files can only be attached to a gallery album."]})
        return attrs
